...
public class HotelAdditionalServicesService {
	...
	public HotelAdditionalServiceDTO getAdditionalService(String name, Hotel hotel) {
		return new HotelAdditionalServiceDTO(hotelAdditionalServicesRepository.findOneByNameAndHotel(name, hotel.getId()));
	}
	...
	public MessageDTO editAdditionalService(HotelAdditionalServiceDTO additionalService, String name, Hotel hotel) {
		...
		if (has == null)
			return new MessageDTO("Hotel additional service with this name does not exist", ToasterType.ERROR.toString());
		if (checkForActiveReservations(has))
			return new MessageDTO("Hotel additional service has reservations bound to it", ToasterType.ERROR.toString());
